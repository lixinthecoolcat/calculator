...
public class Calculator {
   ...
   */
        ...
        while (true) {
            try {
                ...
                if (input.charAt(0) == '-') {
                    ...
                    input = AppUtil.validateInput(input);
    ...
    //Using Integer is a simplify and knowing design choice with assumption of the users for this tool are not bankers
    public static Integer doCalculate(String input) {
    ...
    private static void calculate(Node root) {
        // Post order recursive solution to do calculate
        if (root != null) {
            calculate(root.left);
            calculate(root.right);

            if (root.op != Operator.nil) {
                //root.data is overwritten by each step, since we need final result, we don't need keep intermidiate calculated data
                root.data = root.op.operate(root);
            }
        }
